(function() {
	...
	function showLoading() {
		...
		if (!overlay) overlay = createNewElement("div", body, "overlayLadingPanel");
		overlay.innerHTML = "<div id='preloader'></div>";
		$(overlay).show()
	};

	function createNewElement(element, apTo, id, className) {
	...
	imGaleryView.prototype.loadImage = function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				imagesObj.images.forEach(function(img) {

					var imagePath = imagesObj.urlToSmall + img.small,
						imagePathL = imagesObj.urlToSmall + img.large;
					...
					self.arryOfImageB.push(imagePathL);
					p++;
	 ...
	 */
		...
		document.addEventListener("click", function(event) {
			...
			if (target === "turnLeft") self.moveSlide(-1);
			if (target === "turnRight") self.moveSlide(1);
			if (target === "imageWrap") self.getImage(event.target);
			if (target === "closeBut") self.closeFullScreen();
			if (target === "zoomBut") self.zoomPic(0.25);
			if (target === "zoomOutBut") self.zoomPic(-0.25)
		});
		window.onmousedown = function(event) {
			var e = event.target.className;
			if (e.search(/infoScreenView/) != -1) {
				self.dragMode = true;
				self.dragEl = event.target;
				self.offsetX = event.clientX - event.target.offsetLeft;
				self.offsetY = event.clientY - event.target.offsetTop
			}
		};
		...
		window.onkeydown = function(e) {
			...
			if (a == 37) self.moveSlide(-1);
			if (a == 39) self.moveSlide(1);
			if (a == 27) self.closeFullScreen()
		};
		...
		window.onmousemove = function(event) {
			...
			self.mouseMove(e);
			if (self.dragMode) self.infoScreenMove(event.clientX, event.clientY)
		};
	...
	imGaleryView.prototype.showGalery = function() {
		...
		setTimeout(function() {
			...
			createNewElement("div", body, false, "turnLeft");
			createNewElement("div", body, false, "turnRight");
			createNewElement("div", body, false, "closeBut");
			createNewElement("div", body, "infoScreen");
			var infoScreen = document.getElementById('infoScreen');
			createNewElement("div", infoScreen, false, "infoScreenOverlay");
			createNewElement("div", infoScreen, false, "infoScreenView");
			var tools = createNewElement("div", body, "toolsContainer");
			createNewElement("div", tools, false, "zoomBut");
			createNewElement("div", tools, false, "zoomOutBut");
	...
	this.imGaleryView = imGaleryView;
}).call(this);